...
import com.sample.payment_server.ExceptionClass.InsufficientBalanceException;
import com.sample.payment_server.ExceptionClass.InvalidAccountsException;
import com.sample.payment_server.ExceptionClass.SelfTransferException;
...
public class PaymentService {
    ...
    public String transfer(String senderEmail,Long receiverWalletId,double amount,String description) {
        ...
        if(sender == null || receiver == null)throw new InvalidAccountsException("Provide valid accounts");
        if(sender.getAmount() < amount)throw new InsufficientBalanceException("Insufficient balance");
        if(sender.getUserId().equals(receiver.getUserId()))throw new SelfTransferException("Cant transfer to yourself");

        TransferEntity transaction = new TransferEntity(
                sender.getWalletId(), receiver.getWalletId(), sender.getUsername(),
                receiver.getUsername(), sender.getUserId(), receiver.getUserId(),
                amount, LocalDateTime.now(), TransferStatus.PENDING, description
        );
        ...
        try{
            ...
            WalletUpdateRequestDTO senderWallet = new WalletUpdateRequestDTO(sender.getWalletId(),senderAmount,sender.getEmail());
            WalletUpdateRequestDTO reciverWallet = new WalletUpdateRequestDTO(receiver.getWalletId(), receiverAmount,receiver.getEmail());

            // if no response , roll back (set time out)
